...
6. *The non-Touch Bar model is slower than last year's 13" MacBook Pro* In base frequency, yes. It's a 15W part _replacing_ a 28W part. That said, its turbo clock speed is the same (3.1GHz) and in some preliminary benchmarks it is performing equivalently. In many cases the performance differential will simply not be noticeable if even existent at all (particularly with a newer generation Intel iGPU). This model is designed to attract people from the Air (15W tier) and offers significantly more than the outgoing Air at a small price premium.
