...
6. *The Non-Touch Bar model is slower than last year's 13" MacBook Pro*. In base frequency, yes. It's a 15W part "replacing" a 28W part. That said, its turbo clock speed is the same and in some preliminary benchmarks it is performing admirably. In most cases the performance differential will simply not be noticeable if even existent at all (particularly with a newer generation Intel GPU)
